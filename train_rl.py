...
def load_denoiser(encoder, device, dynamic_dim):
    """Optionally load a pretrained denoiser for feature denoising."""
    ...
    if not os.path.exists(denoiser_path):
        print(f"WARNING: {denoiser_path} not found. "
              f"Denoiser will be randomly initialized and frozen.")
        ...
        denoiser.load_state_dict(
            torch.load(denoiser_path, map_location=device, weights_only=False))
    ...
    print("Denoiser attached to encoder (frozen).")
...
def main():
    ...
    # Initialize encoder from scratch (no pretrained competition head)
    ...
    # Optionally load frozen denoiser
    load_denoiser(encoder, device, dynamic_dim)
    ...
    # Train encoder + policy end-to-end with GRPO from the start
